...
.svc-rows { display: flex; flex-direction: column; gap: 8px; max-height: 200px; overflow-y: auto; margin-bottom: 4px; }
.svc-row { display: flex; align-items: center; gap: 8px; }
.svc-row input { flex: 1; padding: 7px 10px; border: 1.5px solid var(--border2); border-radius: var(--radius-sm); font-family: 'DM Sans', sans-serif; font-size: 13px; color: var(--text); background: var(--surface); outline: none; }
  ...
  <div class="main">
          ...
          <div id="form-body-wrap">
            <div class="form-body" id="form-body">
              ...
              <div class="field" id="f-servico"><label>Serviço de interesse</label>
                <select data-key="servico" id="form-servico-select">
    ...
    <div class="panel" id="view-editor">
      <div class="editor-grid" style="width:100%;max-width:860px;margin:0 auto">
        ...
        <div class="editor-section">
          <h3>Serviços do formulário</h3>
          <div class="svc-rows" id="svc-rows"></div>
          <button class="btn-add" onclick="addSvcRow()">+ novo serviço</button>
        </div>
        <div class="editor-section">
...
function renderSvcRows() {
  const container = document.getElementById('svc-rows');
  if (!container) return;
  container.innerHTML = '';
  state.services.forEach((svc, i) => {
    ...
    row.innerHTML = `
      <input type="text" value="${svc.replace(/"/g,'&quot;')}" oninput="updateSvc(${i},this.value)">
      <div class="col-del" onclick="removeSvc(${i})" title="Remover">✕</div>`;
    container.appendChild(row);
  });
}
function updateSvc(i, val) { state.services[i] = val; renderServiceOptions(); }
function addSvcRow() { state.services.push('Novo serviço'); renderSvcRows(); renderServiceOptions(); }
function removeSvc(i) { state.services.splice(i, 1); renderSvcRows(); renderServiceOptions(); }
function renderServiceOptions() {
  const sel = document.getElementById('form-servico-select');
  if (!sel) return;
  sel.innerHTML = '<option value="">Selecione um serviço...</option>';
  state.services.forEach(s => { const o = document.createElement('option'); o.textContent = s; sel.appendChild(o); });
}
...
async function submitForm() {
  ...
  try {
    ...
    // Redireciona pro WhatsApp após 1.5s
    setTimeout(() => {
